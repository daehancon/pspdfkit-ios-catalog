...
private class SnakeViewController: PDFViewController {
    ...
    func addTail() {
        guard let firstTwoPoints = snake.points?.prefix(2) else { return }
        guard let firstPoint = snake.points?.first else { return }
        let secondPoint = firstTwoPoints[1]
        var yFirst = Int(firstPoint.y)
        var xFirst = Int(firstPoint.x)
        ...
        snake.points?.insert(CGPoint(x: xFirst, y: yFirst), at: 0)
    }
