...
/// Once the PDF is cached, this class simply forwards to the PSPDFKit built-in image loader.
final class StreamingImageLoader: NSObject, PDFPageCellImageLoading {
    ...
    func requestImageForPage(at pageIndex: PageIndex, availableSize size: CGSize, completionHandler: @escaping (UIImage?, Error?) -> Void) -> PDFPageCellImageRequestToken {
        ...
        // No image cached and no PDF there. We need to downoad the file.
    ...
    static func configure(cell: PDFPageCell?, documentFetcher: @escaping () -> Document?) {
        guard let cell = cell, let imageLoader = cell.imageLoader else { return }
